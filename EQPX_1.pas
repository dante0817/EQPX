...
type
  TfrmMain = class(TForm)
    ...
    btnSetDate: TButton;
    btnAutoBrute: TButton;
    btnMagnitude: TButton;
    ...
    procedure btnAutoBruteClick(Sender: TObject);
    procedure btnMagnitudeClick(Sender: TObject);
...
uses UTILS_Brute_Force, UTILS_DB, UTILS_fileIO, UTILS_settings, UTILS_DateTime, UTILS_ATLAS,
...
procedure TfrmMain.btnAutoBruteClick(Sender: TObject);
var
  mask, bestMask, maxMask: Cardinal;
  bestRMS, rms: Double;
begin
  UTILS_Brute_Force.BruteForceSearch;

  exit;

  UTILS_Brute_Force.BuildPhaseList;                 // collect every in-grid pick
  if Length(PhaseList) = 0 then Exit;

  // build PHILSTA.DAT once
  ...
  // save pristine grid so we can restore quickly
  var OriginalRows := TStringList.Create;
  try
    OriginalRows.Assign(sgMainData.Rows[0]);
    for var r := 1 to sgMainData.RowCount-1 do
      OriginalRows.Add(sgMainData.Rows[r].Text);

    for mask := 1 to maxMask do
    begin
      if not UTILS_Brute_Force.MaskIsValid(mask) then Continue;

      // restore grid then apply mask
      for var r := 0 to sgMainData.RowCount-1 do
        sgMainData.Rows[r].DelimitedText := OriginalRows[r];

      UTILS_Brute_Force.ApplyMask(mask);

      PLOT_DATClick(nil);   // write new PLOT.DAT
      PlotClick(nil);       // run plotter + parse RMS

      rms := StrToFloatDef(ledRMS.Text, 1E30);
      if rms < bestRMS then
      begin
        bestRMS  := rms;
        bestMask := mask;
    ...
    // final: restore pristine grid and apply best solution
    for var r := 0 to sgMainData.RowCount-1 do
      sgMainData.Rows[r].DelimitedText := OriginalRows[r];
    ApplyMask(bestMask);
    PLOT_DATClick(nil);
    ...
    ShowMessage(Format('Auto-Brute complete — best RMS = %.3f',[bestRMS]));
  finally
    OriginalRows.Free;
  end;
end;
...
procedure TfrmMain.btnMagnitudeClick(Sender: TObject);
var
  i: Integer;
  Station: string;
  Duration, Delta, Mag: Single;
begin
  for i := 1 to frmMain.sgMainData.RowCount - 1 do
  begin
    Station := frmMain.sgMainData.Cells[0, i];

    if TryStrToFloat(frmMain.sgMainData.Cells[7, i], Duration) and
       TryStrToFloat(frmMain.sgMainData.Cells[8, i], Delta) then
    begin
      Mag := UTILS_Math.ComputeMagnitude(Station, Duration, Delta);
      frmMain.sgMainData.Cells[9, i] := FormatFloat('0.00', Mag);
    end;
  end;

  UTILS_Math.RemoveOutliersAndComputeMean;
end;
