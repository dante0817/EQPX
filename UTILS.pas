...
procedure Initialize_sgMainData_Headers;
...
begin
  ...
  begin
    ColCount := 10;  // Total columns: 0..7
    RowCount := 2;  // Minimum rows: header + 1 data row

    // Set header row (row 0)
//    Cells[0, 0] := 'Station';      // Station code
    Cells[1, 0] := 'P-Time';       // P-phase arrival time (min.sec)
    Cells[2, 0] := 'P-Res';        // P-phase residual
    Cells[3, 0] := 'P-Marker';     // '*' or blank
    Cells[4, 0] := 'S-Time';       // S-phase arrival time (min.sec)
    Cells[5, 0] := 'S-Res';        // S-phase residual
    Cells[6, 0] := 'S-Marker';     // '*' or blank
    Cells[7, 0] := 'Coda';         // Duration or end of event indicator
    Cells[8, 0] := 'Delta';
    Cells[9, 0] := 'Magnitude';
  end;
end;
